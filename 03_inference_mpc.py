...
def multiview_chw_to_grid_bgr(img_multi_chw: np.ndarray, scale: int = 1) -> np.ndarray:
    ...
    views = np.split(img_multi_chw, num_views, axis=0)
    views_bgr = [chw_rgb_to_hwc_bgr(v) for v in views]
    ...
    rows_imgs = []
    for r in range(rows):
        rows_imgs.append(np.concatenate(views_bgr[r * cols:(r + 1) * cols], axis=1))
...
def grab_frame_np(renderer, data):
    """Return concatenated multiview CHW uint8 RGB."""
...
def grab_frame(renderer, data):
    img = grab_frame_np(renderer, data)
    return torch.tensor(img).float().to(DEVICE) / 255.0

# --- HÀM QUAN TRỌNG: LẤY ẢNH GOAL ---
def capture_goal_image(env, renderer):
    """
    ...
    # Cần update scene thì thay đổi màu sắc mới có hiệu lực
    # Chụp ảnh goal (multiview)
    img_goal_chw = grab_frame_np(renderer, data)
    ...
    return img_tensor.unsqueeze(0), img_goal_chw  # [1, C, H, W], CHW uint8
...
def run_inference():
    ...
    if PREVIEW:
        ...
        cv2.resizeWindow(
            "mpc_preview",
            IMG_SIZE[0] * cols * PREVIEW_SCALE * 2 + 10,
            IMG_SIZE[1] * rows * PREVIEW_SCALE,
        )
    ...
    for ep in range(NUM_EPISODES):
        ...
        img_goal, img_goal_chw = capture_goal_image(env, renderer)
        with torch.no_grad():
            z_goal = model["encoder"](img_goal)

        if PREVIEW:
            goal_vis = multiview_chw_to_grid_bgr(img_goal_chw, scale=PREVIEW_SCALE)

        print(f"🚀 [ep={ep}] Start Control Loop...")
        for step in range(MAX_STEPS):
            img_curr_chw = grab_frame_np(renderer, data)
            ...
            if PREVIEW:
                curr_vis = multiview_chw_to_grid_bgr(img_curr_chw, scale=PREVIEW_SCALE)
