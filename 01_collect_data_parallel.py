...
def grab_frame(renderer, data, img_size):
    """Render all cameras and concatenate into CHW (3 * NUM_CAMERAS, H, W)."""
    ...
    try:
        ...
        for _ep in iterator:
            try:
                ...
                img_t = grab_frame(renderer, data, COMMON.img_size)
                for _ in range(COLLECT.max_steps):
                    ...
                    img_next = grab_frame(renderer, data, COMMON.img_size)
...
def merge_shards(shard_results, out_dir):
    ...
    img_channels = 3 * NUM_CAMERAS
    obs_mm = np.lib.format.open_memmap(obs_out, mode="w+", dtype=np.uint8, shape=(total, img_channels, h, w))
    actions_mm = np.lib.format.open_memmap(actions_out, mode="w+", dtype=np.float32, shape=(total, 4))
    next_obs_mm = np.lib.format.open_memmap(next_obs_out, mode="w+", dtype=np.uint8, shape=(total, img_channels, h, w))
