...
# Cameras to render for multiview capture
CAMERAS = ["corner", "corner2", "corner3", "topview"]
...
def grab_frame(renderer, data, img_size=IMG_SIZE):
    """
    Grab multiview frame and return concatenated CHW (3 * NUM_CAMERAS, H, W).
    """
    def render_cam(camera):
    ...
    imgs = np.concatenate([render_cam(cam) for cam in CAMERAS], axis=0)
    return imgs.astype(np.uint8, copy=False)
...
def multiview_chw_to_grid_bgr(img_multi_chw: np.ndarray, scale: int = 1) -> np.ndarray:
    """
    Convert concatenated multiview CHW to a BGR grid for preview.
    """
    if img_multi_chw.shape[0] % 3 != 0:
        raise ValueError("Expected channel dimension to be multiple of 3 for RGB views")

    num_views = img_multi_chw.shape[0] // 3
    views = np.split(img_multi_chw, num_views, axis=0)  # [(3, H, W), ...]
    views_bgr = [chw_rgb_to_hwc_bgr(view) for view in views]

    # Arrange in near-square grid (2x2 for 4 cams, 1xN otherwise)
    cols = int(np.ceil(np.sqrt(num_views)))
    rows = int(np.ceil(num_views / cols))

    # Pad with black images if needed to fill grid
    h, w, _ = views_bgr[0].shape
    blank = np.zeros_like(views_bgr[0])
    while len(views_bgr) < rows * cols:
        views_bgr.append(blank)

    rows_imgs = []
    for r in range(rows):
        row_imgs = views_bgr[r * cols:(r + 1) * cols]
...
def collect():
    ...
    if STREAM_TO_NPY:
        ...
        # grab_frame returns CHW (3 * NUM_CAMERAS, H, W). IMG_SIZE is (W, H) in cv2.resize().
        h, w = IMG_SIZE[1], IMG_SIZE[0]
        img_channels = 3 * NUM_CAMERAS
        obs_mm = np.lib.format.open_memmap(obs_path, mode="w+", dtype=np.uint8, shape=(max_transitions, img_channels, h, w))
        actions_mm = np.lib.format.open_memmap(actions_path, mode="w+", dtype=np.float32, shape=(max_transitions, 4))
        next_obs_mm = np.lib.format.open_memmap(next_obs_path, mode="w+", dtype=np.uint8, shape=(max_transitions, img_channels, h, w))
    else:
    ...
    if PREVIEW:
        ...
        preview_cols = int(np.ceil(np.sqrt(NUM_CAMERAS)))
        preview_rows = int(np.ceil(NUM_CAMERAS / preview_cols))
        cv2.resizeWindow(
            "preview",
            IMG_SIZE[0] * preview_cols * PREVIEW_SCALE,
            IMG_SIZE[1] * preview_rows * PREVIEW_SCALE,
        )
    ...
    try:
        for ep in tqdm(range(NUM_EPISODES)):
            try:
                ...
                # Render frame đầu (multiview)
                img_t = grab_frame(renderer, data)
                if PREVIEW:
                    vis = multiview_chw_to_grid_bgr(img_t, scale=PREVIEW_SCALE)
                ...
                for _ in range(MAX_STEPS):
                    ...
                    img_next = grab_frame(renderer, data)
                    if PREVIEW:
                        vis = multiview_chw_to_grid_bgr(img_next, scale=PREVIEW_SCALE)
                        cv2.imshow("preview", vis)
                        if (cv2.waitKey(PREVIEW_WAIT_MS) & 0xFF) in (ord("q"), 27):
                            raise KeyboardInterrupt
